...
%     % ---------------------------------------
%     % Introduced by Vasco Curdia, 06/06/22
%     if eu(1)==0
%         length(bigev)
%         nunstab
%     end
...
if isempty(veta1)
	...
	[uetaLoose,detaLoose,vetaLoose] = svd(veta1-veta*veta'*veta1);
	bigevLoose = find(diag(detaLoose) > realsmall);
	unique= (length(bigevLoose) == 0);
%     LooseEV = diag(detaLoose);LooseEV(bigevLoose)
end
if unique
